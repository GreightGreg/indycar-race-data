      ...
      <div className="min-h-screen bg-racing-bg flex items-center justify-center">
        <div className="bg-racing-surface rounded p-8 w-full max-w-sm">
          ...
          <input type="password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()}
            placeholder="Enter admin password" className="w-full bg-racing-bg border border-racing-border text-racing-text font-mono text-sm px-3 py-2 rounded mb-3" />
          {error && <p className="text-racing-red font-mono text-xs mb-3">{error}</p>}
  ...
  return (
    <div className="min-h-screen bg-racing-bg">
      <header className="bg-racing-bg border-b-2 border-racing-yellow px-4 py-3">
        ...
        <p className="font-condensed text-[10px] text-racing-muted uppercase tracking-[0.2em]">RACE DATA ADMIN</p>
      </header>
      ...
      <div className="max-w-[1000px] mx-auto px-4 py-6 space-y-8">
        ...
        <section className="bg-racing-surface rounded p-6">
          ...
          <div className="flex gap-3 items-end mb-4">
            <div>
              <label className="font-condensed text-xs text-racing-muted block mb-1">Year</label>
              <input type="number" value={seasonYear} onChange={e => setSeasonYear(Number(e.target.value))}
                className="bg-racing-bg border border-racing-border text-racing-text font-mono text-sm px-3 py-2 rounded w-24" />
            </div>
            <div>
              <label className="font-condensed text-xs text-racing-muted block mb-1">Total Rounds</label>
              <input type="number" value={totalRounds} onChange={e => setTotalRounds(Number(e.target.value))}
                className="bg-racing-bg border border-racing-border text-racing-text font-mono text-sm px-3 py-2 rounded w-20" />
            </div>
          ...
          {seasons.length > 0 && (
            <table className="w-full text-left">
              <thead><tr className="border-b border-racing-border">
                <th className="font-condensed text-xs text-racing-muted px-2 py-1">Year</th>
                <th className="font-condensed text-xs text-racing-muted px-2 py-1">Rounds</th>
              </tr></thead>
              <tbody>{seasons.map(s => (
                <tr key={s.id} className="border-b border-racing-border/30">
                  <td className="px-2 py-1 font-mono text-sm text-racing-text">{s.year}</td>
                  <td className="px-2 py-1 font-mono text-sm text-racing-text">{s.total_rounds}</td>
                </tr>
              ))}</tbody>
            </table>
          )}
        ...
        <section className="bg-racing-surface rounded p-6">
          ...
          <p className="font-mono text-[10px] text-racing-muted mb-3">Drop race weekend PDFs here — upload any or all reports in any order</p>
          <input type="file" accept=".pdf" multiple onChange={e => setFiles(Array.from(e.target.files || []))}
            className="w-full bg-racing-bg border border-racing-border text-racing-text font-mono text-sm px-3 py-2 rounded mb-3 file:bg-racing-blue file:text-white file:border-0 file:rounded file:px-3 file:py-1 file:mr-3 file:font-condensed" />
          {files.length > 0 && <p className="font-mono text-xs text-racing-muted mb-3">{files.length} file(s) selected</p>}
            ...
            <div className="space-y-3">
              {pendingRaces.map(r => (
                <div key={r.id} className="bg-racing-bg rounded p-4 border border-racing-border">
                  <p className="font-condensed text-sm text-racing-text">{r.event_name} — {r.track_name} — Round {r.round_number}</p>
                  <p className="font-mono text-[10px] text-racing-muted mt-1">Status: Pending — waiting for all reports</p>
                </div>
              ))}
            </div>
          </section>
            ...
            <table className="w-full text-left">
              <thead><tr className="border-b border-racing-border">
                {['Round','Event','Track','Date','Actions'].map(h => (
                  <th key={h} className="font-condensed text-xs text-racing-muted uppercase px-3 py-2">{h}</th>
                ))}
              </tr></thead>
              <tbody>
                {races.map(r => (
                  <tr key={r.id} className="border-b border-racing-border/30">
                    <td className="px-3 py-2 font-mono text-sm text-racing-text">{r.round_number}</td>
                    <td className="px-3 py-2 font-body text-sm text-racing-text">{r.event_name}</td>
                    <td className="px-3 py-2 font-body text-sm text-racing-text">{r.track_name}</td>
                    <td className="px-3 py-2 font-mono text-xs text-racing-muted">{r.race_date}</td>
                    <td className="px-3 py-2">
                      <button onClick={() => handleDelete(r.id)} className="font-condensed text-xs text-racing-red hover:underline">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
