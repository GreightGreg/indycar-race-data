    ...
    <div className="space-y-8">
        ...
        <div className="space-y-1.5">
          {sessionStats?.map(s => (
            <div key={s.id} className={`rounded p-3 ${s.session_type === 'Totals' ? 'bg-racing-surface2 border border-racing-border' : 'bg-racing-surface'}`}>
              <p className="font-condensed text-[15px] text-racing-text">{s.session_type}</p>
              <div className="flex gap-4 mt-1">
                <span className="font-mono text-sm text-racing-yellow">{s.laps.toLocaleString()} laps</span>
                <span className="font-mono text-sm text-racing-text">{Number(s.miles).toFixed(0)} mi</span>
                <span className="font-mono text-sm text-racing-muted">{s.time_on_track}</span>
              </div>
            </div>
          ))}
        </div>
            ...
            <thead>
              <tr className="border-b border-racing-border">
                {['Session','Laps','Miles','Time on Track'].map(h => (
                  <th key={h} className="font-condensed font-semibold text-sm text-racing-muted uppercase px-3 py-2">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {sessionStats?.map(s => (
                <tr key={s.id} className={`border-b border-racing-border/50 ${s.session_type === 'Totals' ? 'bg-racing-surface' : ''}`}>
                  <td className="px-3 py-2 font-condensed text-[15px] text-racing-text">{s.session_type}</td>
                  <td className="px-3 py-2 font-mono text-sm text-racing-yellow">{s.laps.toLocaleString()}</td>
                  <td className="px-3 py-2 font-mono text-sm text-racing-text">{Number(s.miles).toFixed(2)}</td>
                  <td className="px-3 py-2 font-mono text-sm text-racing-text">{s.time_on_track}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="bg-racing-surface rounded border-t-2 border-racing-blue p-4">
          <p className="font-condensed text-sm text-racing-muted uppercase">Most Improved</p>
          ...
          <p className="font-mono text-[12px] text-racing-muted">Gained {race.most_improved_positions ?? '—'} positions</p>
        </div>
        <div className="bg-racing-surface rounded border-t-2 border-racing-yellow p-4">
          <p className="font-condensed text-sm text-racing-muted uppercase">Best Race Lap</p>
          <p className="font-heading text-xl text-racing-yellow mt-1">{formatDriverName(race.fastest_lap_driver)} #{race.fastest_lap_car}</p>
          <p className="font-mono text-[12px] text-racing-muted">{race.fastest_lap_speed} mph · {race.fastest_lap_time} sec · Lap {race.fastest_lap_number}</p>
        </div>
        <div className="bg-racing-surface rounded border-t-2 border-racing-blue p-4">
          <p className="font-condensed text-sm text-racing-muted uppercase">Best Lead Lap</p>
          <p className="font-heading text-xl text-racing-yellow mt-1">{formatDriverName(race.best_lead_lap_driver)}</p>
          <p className="font-mono text-[12px] text-racing-muted">{race.best_lead_lap_speed} mph · {race.best_lead_lap_time} sec</p>
        </div>
        {combinedPractice?.[0] && (
          <div className="bg-racing-surface rounded border-t-2 border-racing-yellow p-4">
            <p className="font-condensed text-sm text-racing-muted uppercase">Combined Practice Best</p>
            ...
            <p className="font-mono text-[12px] text-racing-muted">{Number(combinedPractice[0].best_speed).toFixed(3)} mph · {combinedPractice[0].best_session}</p>
          </div>
        )}
      </div>
            ...
            <div className="space-y-1.5">
              {combinedPractice.map(d => (
                <div key={d.id} className="bg-racing-surface rounded p-3 flex items-center gap-2.5">
                  <span className="font-heading text-[15px] text-racing-muted w-5 shrink-0">{d.rank}</span>
                  <CarBadge num={d.car_number} />
                  <div className="min-w-0 flex-1">
                    <p className="font-body text-[15px] text-racing-text">{formatDriverName(d.driver_name)}</p>
                    <div className="flex gap-3 mt-0.5">
                      <span className="font-mono text-[12px] text-racing-yellow">{d.best_time}s</span>
                      <span className="font-mono text-[12px] text-racing-text">{Number(d.best_speed).toFixed(3)} mph</span>
                      <span className="font-mono text-[12px] text-racing-muted">{d.best_session}</span>
                    </div>
                  </div>
                </div>
              ))}
                ...
                <tbody>
                  {combinedPractice.map(d => (
                    <tr key={d.id} className="border-b border-racing-border/50">
                      <td className="px-3 py-2 font-heading text-[15px] text-racing-muted">{d.rank}</td>
                      <td className="px-3 py-2"><CarBadge num={d.car_number} /></td>
                      <td className="px-3 py-2 font-body text-[15px] text-racing-text">{formatDriverName(d.driver_name)}</td>
                      <td className="px-3 py-2"><EngineIcon engine={d.engine || ''} /></td>
                      <td className="px-3 py-2 font-mono text-sm text-racing-yellow">{d.best_time}s</td>
                      <td className="px-3 py-2 font-mono text-sm text-racing-text">{Number(d.best_speed).toFixed(3)} mph</td>
                      <td className="px-3 py-2 font-mono text-sm text-racing-muted">{d.best_session}</td>
                      <td className="px-3 py-2 font-mono text-sm text-racing-muted">{d.total_laps}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                return (
                  <div key={q.id} className={`bg-racing-surface rounded p-3 ${isDNQ ? 'opacity-70' : ''}`}>
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className="font-heading text-[15px] text-racing-muted w-6 shrink-0">P{q.qual_position}</span>
                      <CarBadge num={q.car_number} />
                      <span className="font-body text-[15px] text-racing-text">{formatDriverName(q.driver_name)}</span>
                      <EngineIcon engine={q.engine || ''} />
                      {isDNQ && <span className="ml-auto inline-flex items-center px-1.5 py-0.5 rounded text-[12px] font-heading bg-racing-orange/20 text-racing-orange">DNQ</span>}
                    </div>
                  ...
                  {qualResults.map(q => {
                    ...
                    return (
                      <tr key={q.id} className={`border-b border-racing-border/50 ${isDNQ ? 'opacity-70' : ''}`}>
                        <td className="px-3 py-2 font-heading text-[15px] text-racing-muted">P{q.qual_position}</td>
                        <td className="px-3 py-2"><CarBadge num={q.car_number} /></td>
                        <td className="px-3 py-2 font-body text-[15px] text-racing-text">{formatDriverName(q.driver_name)}</td>
                        <td className="px-3 py-2"><EngineIcon engine={q.engine || ''} /></td>
                        <td className={`px-3 py-2 font-mono text-sm ${fasterLap === 1 ? 'text-racing-yellow font-bold' : 'text-racing-text'}`}>
                          {q.lap1_time ? `${q.lap1_time}s` : 'No Time'}
                        </td>
                        <td className={`px-3 py-2 font-mono text-sm ${fasterLap === 2 ? 'text-racing-yellow font-bold' : 'text-racing-text'}`}>
                          {q.lap2_time ? `${q.lap2_time}s` : isDNQ ? 'DNQ' : 'No Time'}
                        </td>
                        <td className="px-3 py-2 font-mono text-sm text-racing-text">{q.total_time ? `${q.total_time}s` : '—'}</td>
                        <td className="px-3 py-2 font-mono text-sm text-racing-text">{q.avg_speed ? `${Number(q.avg_speed).toFixed(3)} mph` : '—'}</td>
                        <td className="px-3 py-2">
                          {isDNQ && <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[12px] font-heading bg-racing-orange/20 text-racing-orange">DNQ</span>}
                        </td>
                      </tr>
                    );
                  })}
        ...
        <div className="space-y-1.5">
          {data.map(d => (
            <div key={d.id} className="bg-racing-surface rounded p-2.5 flex items-center gap-2">
              <span className="font-heading text-[15px] text-racing-muted w-5 shrink-0">{d.rank}</span>
              <span className="inline-flex items-center justify-center bg-racing-blue text-white font-heading text-sm w-7 h-5 rounded-sm shrink-0">{d.car_number}</span>
              <div className="min-w-0 flex-1">
                <p className="font-body text-sm text-racing-text truncate">{formatDriverName(d.driver_name)}</p>
                <div className="flex gap-2 mt-0.5">
                  <span className="font-mono text-[12px] text-racing-yellow">{d.best_time ? `${d.best_time}s` : 'DNP'}</span>
                  <span className="font-mono text-[12px] text-racing-text">{d.best_speed ? `${Number(d.best_speed).toFixed(3)} mph` : '—'}</span>
                  <span className="font-mono text-[12px] text-racing-muted">{d.total_laps} laps</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[600px] text-left">
            ...
            <tbody>
              {data.map(d => (
                <tr key={d.id} className="border-b border-racing-border/50">
                  <td className="px-2 py-1.5 font-heading text-[15px] text-racing-muted">{d.rank}</td>
                  <td className="px-2 py-1.5"><span className="inline-flex items-center justify-center bg-racing-blue text-white font-heading text-sm w-7 h-5 rounded-sm">{d.car_number}</span></td>
                  <td className="px-2 py-1.5 font-body text-sm text-racing-text">{formatDriverName(d.driver_name)}</td>
                  <td className="px-2 py-1.5"><EngineIcon engine={d.engine || ''} /></td>
                  <td className="px-2 py-1.5 font-mono text-sm text-racing-yellow">{d.best_time ? `${d.best_time}s` : <span className="text-racing-orange">DNP</span>}</td>
                  <td className="px-2 py-1.5 font-mono text-sm text-racing-text">{d.best_speed ? `${Number(d.best_speed).toFixed(3)} mph` : '—'}</td>
                  <td className="px-2 py-1.5 font-mono text-[12px] text-racing-muted">{d.diff_to_leader || '—'}</td>
                  <td className="px-2 py-1.5 font-mono text-[12px] text-racing-muted">{d.gap_to_ahead || '—'}</td>
                  <td className="px-2 py-1.5 font-mono text-[12px] text-racing-muted">{d.best_lap_number ? `L${d.best_lap_number}` : '—'}</td>
                  <td className="px-2 py-1.5 font-mono text-[12px] text-racing-muted">{d.total_laps}</td>
                </tr>
              ))}
            </tbody>
          </table>
