...
const SeasonStatsTab = () => {
  ...
  const SortHeader = ({ k, label, className = '' }: { k: string; label: string; className?: string }) => (
    <th
      ...
      className={`font-condensed font-semibold text-sm text-racing-muted uppercase px-2 py-2 cursor-pointer select-none whitespace-nowrap hover:text-racing-text transition-colors ${sortKey === k ? 'text-racing-yellow' : ''} ${className}`}
    >
  ...
  return (
    <div className="space-y-10">
      ...
      <div>
        ...
        <p className="font-condensed text-[15px] text-racing-muted mt-1">
            ...
            <table className="w-full text-left" style={{ minWidth: `${700 + rounds.length * 48}px` }}>
              <thead>
                <tr className="border-b border-racing-border">
                  ...
                  <th className="font-condensed font-semibold text-sm text-racing-muted uppercase px-2 py-2">Eng</th>
                  {rounds.map(rn => (
              ...
              <tbody>
                {standings.map((s: any, i: number) => (
                  <tr key={s.car} className="border-b border-racing-border/50 hover:bg-racing-surface2/50">
                    ...
                    <td className="px-2 py-2 font-body text-[15px] text-racing-text whitespace-nowrap">{formatDriverName(s.driver_name)}</td>
                    <td className="px-2 py-2"><EngineIcon engine={s.engine} /></td>
                    {rounds.map(rn => (
                      <td key={rn} className={`px-2 py-2 font-mono text-sm text-center ${sortKey === `r_${rn}` ? 'bg-racing-surface2/30' : ''} ${s.roundPoints[rn] !== undefined ? 'text-racing-text' : 'text-racing-muted'}`}>
                        {s.roundPoints[rn] !== undefined ? s.roundPoints[rn] : '—'}
                      </td>
                    ))}
                    <td className={`px-2 py-2 font-mono text-sm font-bold text-racing-yellow ${sortKey === 'totalPts' ? 'bg-racing-surface2/30' : ''}`}>{s.totalPts}</td>
                    <td className={`px-2 py-2 font-mono text-sm text-center ${sortKey === 'wins' ? 'bg-racing-surface2/30' : ''} ${s.wins > 0 ? 'text-racing-yellow' : 'text-racing-muted'}`}>{s.wins}</td>
                    <td className={`px-2 py-2 font-mono text-sm text-center ${sortKey === 'podiums' ? 'bg-racing-surface2/30' : ''} ${s.podiums > 0 ? 'text-racing-text' : 'text-racing-muted'}`}>{s.podiums}</td>
                    <td className={`px-2 py-2 font-mono text-sm text-center ${sortKey === 'top5' ? 'bg-racing-surface2/30' : ''} ${s.top5 > 0 ? 'text-racing-text' : 'text-racing-muted'}`}>{s.top5}</td>
                    <td className={`px-2 py-2 font-mono text-sm text-center ${sortKey === 'lapsLed' ? 'bg-racing-surface2/30' : ''} ${s.lapsLed > 0 ? 'text-racing-text' : 'text-racing-muted'}`}>{s.lapsLed}</td>
                    <td className={`px-2 py-2 font-mono text-sm text-center ${sortKey === 'avgFinish' ? 'bg-racing-surface2/30' : ''} text-racing-text`}>{s.avgFinish}</td>
                    <td className={`px-2 py-2 font-mono text-sm text-center ${sortKey === 'avgStart' ? 'bg-racing-surface2/30' : ''} text-racing-text`}>{s.avgStart}</td>
                    <td className={`px-2 py-2 font-mono text-sm text-center ${sortKey === 'netPos' ? 'bg-racing-surface2/30' : ''} ${s.netPos > 0 ? 'text-racing-green' : s.netPos < 0 ? 'text-racing-red' : 'text-racing-muted'}`}>
            ...
            return (
              <div key={race.id} className="bg-racing-surface rounded border border-racing-border p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="font-condensed text-sm text-racing-muted uppercase">Round {race.round_number}</span>
                    <h4 className="font-heading text-lg text-racing-text">{race.event_name}</h4>
                    <p className="font-condensed text-sm text-racing-muted">{race.track_name} · {race.race_date}</p>
                  </div>
                </div>
                {winner && (
                  <div className="flex items-center gap-2">
                    <span className="font-condensed text-sm text-racing-muted uppercase w-16">Winner</span>
                    <CarBadge num={winner.car_number} />
                    <span className="font-body text-[15px] text-racing-yellow">{formatDriverName(winner.driver_name)}</span>
                    <span className="font-mono text-sm text-racing-muted ml-auto">by {margin}</span>
                  </div>
                )}
...
const MobileStandings = ({ standings, rounds, sortKey, sortDir, onSort }: {
  ...
  <div className="space-y-3">
    ...
    <div className="flex items-center gap-2 flex-wrap">
      <span className="font-condensed text-[12px] text-racing-muted uppercase">Sort by</span>
      {SORT_OPTIONS.map(opt => (
        <button
          key={opt.key}
          onClick={() => onSort(opt.key)}
          className={`font-condensed text-sm px-2 py-1 rounded border transition-colors ${
            sortKey === opt.key
    ...
    {standings.map((s: any, i: number) => (
      <div key={s.car} className="bg-racing-surface rounded border border-racing-border p-3 space-y-2">
        ...
        <div className="flex gap-1 flex-wrap">
          {rounds.map(rn => {
            if (sortKey === 'lapsLed') {
              const led = s.roundLapsLed[rn] ?? 0;
              return (
                <span key={rn} className={`font-mono text-[12px] px-1.5 py-0.5 rounded bg-racing-surface2 ${led > 0 ? 'text-racing-text' : 'text-racing-muted'}`}>
                  R{rn}: {led}
                </span>
              );
            }
...
const MiniStat = ({ label, value, highlight, positive, negative }: {
  ...
  <div className="text-center">
    <p className="font-condensed text-[11px] text-racing-muted uppercase">{label}</p>
...
const Leaderboard = ({ title, subtitle, items }: { title: string; subtitle?: string; items: LeaderboardItem[] }) => (
  <div className="bg-racing-surface rounded border border-racing-border p-4">
    ...
    {subtitle && <p className="font-condensed text-[12px] text-racing-muted mb-3">{subtitle}</p>}
    {!items.length ? (
      <p className="text-racing-muted font-body text-sm py-2">No data yet</p>
    ) : (
      <div className="space-y-2">
        {items.map(item => (
          <div key={item.car} className="flex items-center gap-2">
            <span className="font-heading text-[15px] text-racing-muted w-5 text-right">{item.rank}</span>
            <CarBadge num={item.car} size="sm" />
            <span className="font-body text-sm text-racing-text truncate flex-1">{formatDriverName(item.driver)}</span>
            <div className="text-right shrink-0">
              <span className="font-mono text-sm text-racing-yellow">{item.stat}</span>
              {item.sub && <p className="font-condensed text-[11px] text-racing-muted">{item.sub}</p>}
            </div>
          </div>
        ))}
      </div>
