...
const TABS = ['Race Results','Session Results','Position Chart','Laps Led','Head to Head','Pit Strategy','Track Dominance','Season Stats','Championship','Season Overview'];
...
const TabNav = ({ activeTab, onTabChange }: TabNavProps) => {
  ...
  if (isMobile) {
    return (
      <nav className="fixed top-[60px] left-0 right-0 z-40 bg-racing-bg border-b border-racing-border" ref={menuRef}>
        <button
